...
setEventCallback(eventRegex.commandsRegex.start, eventRegex.commandsRegexNoName.start, (msg) => {
  if (msgTools.isAuthorized(msg) < 0) {
    ...
    msgTools.sendMessage(bot, msg, 'Happy mirroring 😉', -1);
  }
});
...
function cancelMirror(dlDetails: details.DlVars, cancelMsg?: TelegramBot.Message): boolean {
  if (dlDetails.isUploading) {
    if (cancelMsg) {
      msgTools.sendMessage(bot, cancelMsg, 'Upload in Progres, You Cannot Cancel');
    }
    return false;
  } else {
  ...
  if (dlDetails) {
    ...
    if (!statusInterval) {
      statusInterval = setInterval(updateAllStatus,
        constants.STATUS_UPDATE_INTERVAL_MS ? constants.STATUS_UPDATE_INTERVAL_MS : 5000);
    }
  } else if (retry <= 8) {
...
function initAria2(): void {
  ariaTools.openWebsocket((err) => {
    if (err) {
      ...
      console.log('A2C: WebSocket Opened. Bot Ready ... 👍✌️👍');
    }
  });
